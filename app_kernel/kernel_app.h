...
bool setup_task(QCTask *task, vector<int> &kernel)
{
	...
	//actually is use the kernel comes from qc_app, all must be in 2hop
	//add all mutual 2hop neighbors
	k_2hop = global_g.mpplvl2_nbs[k_id];
	for (int i = 1; i <= k_2hop[0]; i++)
	{
		hop2_hit_count[k_2hop[i]]++;
		//all vertices to construct new pvertices should be 2 hop of kernel
	...
	for (int i = 0; i < k_size - 1; i++)
	{
		...
		in_count = 0;
		ext_count = 0;
		int *k_1hop_o = global_g.mppadj_lists_o[k_id];
		for (int j = 1; j <= k_1hop_o[0]; j++)
		{
			int vid = k_1hop_o[j];
		...
		pnew_vertices[i].ncand_deg_o = ext_count;
		pnew_vertices[i].nclique_deg_o = in_count;

		//set ex_deg_i
		in_count = 0;
		ext_count = 0;
		int *k_1hop_i = global_g.mppadj_lists_i[k_id];
		for (int j = 1; j <= k_1hop_i[0]; j++)
		{
			int vid = k_1hop_i[j];
			if (hop2_hit_count[vid] == k_size)
				ext_count++;
		...
		pnew_vertices[i].ncand_deg_i = ext_count;
		pnew_vertices[i].nclique_deg_i = in_count;
	...
	pnew_vertices[k_size - 1].ncand_deg_o = ext_count;
	pnew_vertices[k_size - 1].nclique_deg_o = in_count;
	...
	pnew_vertices[k_size - 1].ncand_deg_i = ext_count;
	pnew_vertices[k_size - 1].nclique_deg_i = in_count;
	...
	for (int i = k_size; i < id_array.size(); i++)
	{
		...
		in_count = 0;
		ext_count = 0;
		int *cand_1hop_o = global_g.mppadj_lists_o[cand_id];
		for (int j = 1; j <= cand_1hop_o[0]; j++)
		{
			int vid = cand_1hop_o[j];
			if (hop2_hit_count[vid] == k_size)
				ext_count++;
			else if (hop2_hit_count[vid] == -1)
				in_count++;
		...
		in_count = 0;
		ext_count = 0;
		int *cand_1hop_i = global_g.mppadj_lists_i[cand_id];
		for (int j = 1; j <= cand_1hop_i[0]; j++)
		{
			int vid = cand_1hop_i[j];
			if (hop2_hit_count[vid] == k_size)
				ext_count++;
			else if (hop2_hit_count[vid] == -1)
				in_count++;
...
public:
	...
	virtual void load_data(char *file_path)
	{
		...
		if (global_pvertices != NULL)
		{
			...
			cout<<"spawn_size: "<<spawn_size<<endl;
		}
		id2index_map.clear();
	}
